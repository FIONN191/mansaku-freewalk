...
**[打开手机版 →](https://fionn191.github.io/mansaku-freewalk/)** —— 手机浏览器打开，「添加到主屏幕」就是一个 App，离线可用。

手机上**没法自动发帖**（iOS 的 WKWebView 没有任何 API 能把文件塞进网页的上传框，这是桌面版零压缩上传的根基）。所以手机版的定位是**准备 + 分享**：

- 选图（相册直接选，原文件不压缩）、按类别生成文案、管多个申请包，数据全在本机浏览器里（IndexedDB），不上传任何服务器。
- 「📤 分享」调系统分享面板，把**多图 + 文案**直接送进小红书 / 抖音 App，你在目标 App 里点最后一下发布。
- 或者分别复制三平台格式的文案（`#话题[话题]#` / `#话题` / `#话题#`）+ 保存全部图片到相册，手动贴。
